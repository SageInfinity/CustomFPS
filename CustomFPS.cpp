...
std::vector<IDXGIOutput*> g_vOutputs;
...
void EnumerateOutputs(int adapterIndex, HWND hWidthEdit, HWND hHeightEdit);
...
void UpdateResolutionFields(HWND hWidthEdit, HWND hHeightEdit, int outputIndex) {
	if (g_borderlessFullscreen && outputIndex >= 0 && outputIndex < g_vOutputs.size()) {
		IDXGIOutput* pOutput = g_vOutputs[outputIndex];
...
void EnumerateOutputs(int adapterIndex, HWND hWidthEdit, HWND hHeightEdit) {
	...
	if (adapterIndex >= 0 && adapterIndex < g_vAdapters.size()) {
		IDXGIAdapter* pAdapter = g_vAdapters[adapterIndex];
		IDXGIOutput* pOutput;
		for (UINT i = 0; pAdapter->EnumOutputs(i, &pOutput) != DXGI_ERROR_NOT_FOUND; ++i) {
			g_vOutputs.push_back(pOutput);
			DXGI_OUTPUT_DESC outputDesc;
			pOutput->GetDesc(&outputDesc);
			SendMessage(g_hOutputCombo, CB_ADDSTRING, 0, (LPARAM)outputDesc.DeviceName);
		}
	}
...
LRESULT CALLBACK InputWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE: {
		...
		g_hGpuCombo = CreateWindowA("COMBOBOX", "", CBS_DROPDOWNLIST | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE | CBS_OWNERDRAWFIXED, 50, 150, 300, 30, hWnd, (HMENU)IDC_GPU_COMBO, hInstance, nullptr);
		...
		g_hOutputCombo = CreateWindowA("COMBOBOX", "", CBS_DROPDOWNLIST | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE | CBS_OWNERDRAWFIXED, 50, 210, 300, 30, hWnd, (HMENU)IDC_OUTPUT_COMBO, hInstance, nullptr);
		...
		EnumerateOutputs(0, hWidthEdit_Input, hHeightEdit_Input);
	...
	case WM_COMMAND: {
		if (HIWORD(wParam) == CBN_SELCHANGE) {
			if (LOWORD(wParam) == IDC_GPU_COMBO) {
				int selectedGpuIndex = SendMessage(g_hGpuCombo, CB_GETCURSEL, 0, 0);
				EnumerateOutputs(selectedGpuIndex, hWidthEdit_Input, hHeightEdit_Input);
			}
		...
		if (LOWORD(wParam) == IDC_START_BUTTON) {
			...
			else { g_pSelectedAdapter = nullptr; }

			if (selectedOutputIndex != CB_ERR && selectedOutputIndex < g_vOutputs.size()) {
				g_pSelectedOutput = g_vOutputs[selectedOutputIndex];
			}
			else { g_pSelectedOutput = nullptr; }
...
LRESULT CALLBACK RenderWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_SIZE:
		if (g_pSwapChain && wParam != SIZE_MINIMIZED) {
			...
			CleanupRenderTarget();
			g_pSwapChain->ResizeBuffers(0, g_currentWidth, g_currentHeight, DXGI_FORMAT_UNKNOWN, 0);
			CreateRenderTarget();
		}
		return 0;
	}
